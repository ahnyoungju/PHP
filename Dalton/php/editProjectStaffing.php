...
<?php
  ...
  if( isset($_GET["NUM"]) )
    $strProject_No = $_GET["NUM"];
  if( isset($_POST["strConsultant_Id"]) )
    $strConsultant_Id = $_POST["strConsultant_Id"];
  if( isset($_GET["ID"]) )
    $strConsultant_Id = $_GET["ID"];

?>
<section>EDIT <?php echo $strProject_No; ?> PROJECT STAFFING DETAILS</section>
<div id="loader"></div>
...
<?php
  ...
  $strDate_Assigned = $strDate_Completed = "";
  ...
  if(isset($_POST["submit"])) {
    ...
    updateStaffing();
    if( $booOk = 1 ) {
      header("Location: ../php/viewProjectStaffing.php?ID=".$strProject_No);
    }
    else
  ...
  // Query to find specific data
  $sql = " SELECT * ";
  $sql.= " FROM d_project_consultant ";
  $sql.= " WHERE Consultant_Id = '".$strConsultant_Id."' AND Project_No = '".$strProject_No."' ";

  readQuery2($sql);
  if($numRecords == 0) {
    displayWarning("Can't find the details!!! ".$strConsultant_Id." ".$strProject_No);
    $booOk = 0;
  }
  else {
    while($arrRows = $stmt->fetch(PDO::FETCH_ASSOC) ){
  ...
  echo "<form action='editProjectStaffing.php' method='POST'>";
  echo "<table class='verticalTBL'>";
  echo "<tr>";
    echo "<th>Consultant Id</th>";
    echo "<td><input type='text' name='Consultant_Id' size='20' ";
    echo " value='$strConsultant_Id' disabled />";
  echo "</td></tr>";
  echo "<tr>";
  ...
  echo "</tr>";
  echo "</table>";
  echo "<td><input type='hidden' name='strConsultant_Id' size='20' value='$strConsultant_Id'/>";
  echo "<td><input type='hidden' name='strProject_No' size='20' value='$strProject_No'/>";
  echo "<input type='submit' class='floatLeft' name='submit' value='Update Details' />";
  echo "</form>";
  echo "<br/>";
